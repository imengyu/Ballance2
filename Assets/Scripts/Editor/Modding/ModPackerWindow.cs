...
你需要先使用“生成”工具来在 Assets/Mods 下生成你的模组包。"), true);
        ...
        private void LoadModsPath()
        {
            ...
            DirectoryInfo direction = new DirectoryInfo(GamePathManager.DEBUG_MOD_FOLDER);
        ...
        private void DoPack()
        {
            ...
            if (tab == 0)
            {
                ...
                {
                    string p = GamePathManager.DEBUG_MOD_FOLDER + "/" + 
                        modsPathArr[selectedMod] + "/ModDef.xml";
                    modDefFile = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
                    if(modDefFile  == null)
                    {
                        isError = true;
                        errStr = "没有在此目录下找到 ModDef.xml ，请先使用生成工具生成";
                        return;
                    }
                }
            }
            ...
            if (!string.IsNullOrEmpty(path))
            {
                ...
                {
                    isError = true;
                    errStr = "ModDef.xml 必须填写包名 packageName";
                    return;
                }
